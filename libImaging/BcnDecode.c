...
#define LOAD32(p) \
	(p)[0] | ((p)[1] << 8) | ((p)[1] << 16) | ((p)[1] << 24)

static void s3tc_color_load(s3tc_color *dst, const UINT8 *src) {
	dst->c0 = LOAD16(src);
	dst->c1 = LOAD16(src + 2);
	dst->lut = LOAD32(src + 4);
}

static void s3tc_alpha_load(s3tc_alpha *dst, const UINT8 *src) {
	memcpy(dst, src, sizeof(s3tc_alpha));
}
...
static void bc7_lerp(rgba *dst, const rgba *a, const rgba *b, int s0, int s1) {
	int t0 = 64 - s0;
	int t1 = 64 - s1;
	dst->r = (UINT8)((t0 * a->r + s0 * b->r) >> 6);
	dst->g = (UINT8)((t0 * a->g + s0 * b->g) >> 6);
	dst->b = (UINT8)((t0 * a->b + s0 * b->b) >> 6);
	dst->a = (UINT8)((t1 * a->a + s1 * b->a) >> 6);
}
...
#undef LOAD
	...
	for (i = 0; i < 16; i++) {
		...
		if (ab) {
			...
			aibit += ib2;
			if (index_sel) {
				bc7_lerp(&col[i], &endpoints[s], &endpoints[s+1], aw[i1], cw[i0]);
			} else {
				bc7_lerp(&col[i], &endpoints[s], &endpoints[s+1], cw[i0], aw[i1]);
			}
		} else {
			bc7_lerp(&col[i], &endpoints[s], &endpoints[s+1], cw[i0], cw[i0]);
		}
#define ROTATE(x, y) \
		val = x; \
		x = y; \
...
#define DECODE_LOOP(NN, SZ, TY, ...) \
	case NN: \
		while (bytes > SZ) { \
			...
			ptr += SZ; \
			bytes -= SZ; \
			if (state->yoff >= state->ysize) return -1; \
		} \
		break
